...
void loop()
{
  ...
  if (ReadCapt("AV") > 2) {  // Si la valeur du capteur avant est supérieure à 1
    Serial.println("Capteur avant activé");
    BtSend("PressionAV"); 
  }

  if (ReadCapt("AR") < 1.5) {  // Si la valeur du capteur arrière est inférieure à 1 (avec resistance)
